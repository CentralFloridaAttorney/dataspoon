...
1. Create a database named 'xyzzydb': xyzzydb = DBTool('xyzzydb').  The table_name will be a default value, unless you specify it during creation (ex. xyzzydb = DBTool('xyzzydb', 'xyzzy_table')) or after creation (ex. xyzzydb.open_table('new_table'))
...
5. Get value using key from row with linking key 'new_linking_key_1': this_value = xyzzydb.get('new_linking_key_1', 'ala')
